...
class Node:
    ...
    def __init__(self):
        ...
        self.__node_depth = 0
        self.__bootstrap: Optional[int] = None
    ...
    @bootstrap.setter
    def bootstrap(self, value: int) -> None:
        self.__bootstrap = value
...
def read_newick_tree(tree_str: str) -> Node:
    ...
    while tree_str[i] != ";":
        if tree_str[i] == "(":
            ...
            elif (sub_str[0] == ":") or (":" in sub_str and (sub_str.split(':')[0]).isdigit()):  # just a branch length or bootstrap and branch length
                bs = eval(sub_str.split(':')[0]) if sub_str.split(':')[0] != '' else None
                bl = eval(sub_str.split(':')[1])
                current_node.branch_length = bl
                if bs is not None and current_node.ancestor:
                    current_node.bootstrap = bs
