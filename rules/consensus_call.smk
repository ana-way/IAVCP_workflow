...
rule getCoverage:
    ...
    output:
        cov = report("results/reports/coverage/{samples}/{samples}_{segment}.samcov.txt", category="BAM covarege", subcategory="{samples}")
    conda:
        "../envs/mapping.yml"
    shell:
        r"""
...
rule consensusCalling:
    input:
        ...
        sortedBam = "{samples}_{segment}_sorted.bam"
    output:
        consensus = "results/consensus/{samples}/{samples}_{segment}_consensus.fa",
        q = temp("results/consensus/{samples}/{samples}_{segment}_consensus.qual.txt")
    conda:
        "../envs/mapping.yml"
    params:
        qual = config["QUALITY"],
