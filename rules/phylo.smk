...
rule treeReconstructio:
     ...
     output:
        ...
        Out7 = temp("{samples}_aligned_nearest_to_{segment}.fasta.uniqueseq.phy"),
...
rule treeRooting:
     ...
     output:
        rtree = "results/trees/{samples}/{samples}_{segment}.midpoint-rooted.nwk"
     conda:
        "../envs/phylo.yml"
     threads:
        8
...
rule treeVisualizing:
     input:
        rtree = "results/trees/{samples}/{samples}_{segment}.midpoint-rooted.nwk"
     output:
        html = report("results/trees/{samples}/{samples}_{segment}.html", category="Phylogenetic", subcategory="{samples}")
     shell:
        r"""
